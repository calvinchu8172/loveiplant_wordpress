...
<h3><?php echo esc_html( apply_filters( 'wc_memberships_members_area_my_membership_notes_title', __( 'My Membership Notes', WC_Memberships::TEXT_DOMAIN ) ) ); ?></h3>

<?php do_action( 'wc_memberships_before_members_area', 'my-memberships-notes' ); ?>

<?php if ( empty ( $customer_notes ) ) : ?>

	<p><?php esc_html_e( 'There are no notes for this membership.', WC_Memberships::TEXT_DOMAIN ); ?> </p>
	...
	<table class="shop_table shop_table_responsive my_account_orders my_account_memberships my_membership_notes">
		...
		<tr>
			...
			$my_membership_notes_columns = apply_filters( 'wc_memberships_members_area_my_membership_notes_column_names', array(
				'membership-note-date'    => __( 'Date', WC_Memberships::TEXT_DOMAIN ),
				'membership-note-time'    => __( 'Time', WC_Memberships::TEXT_DOMAIN ),
				'membership-note-author'  => __( 'Author', WC_Memberships::TEXT_DOMAIN ),
				'membership-note-content' => __( 'Note Content', WC_Memberships::TEXT_DOMAIN ),
			), $user_id );
			?>
		...
		<?php foreach ( $customer_notes as $note ) : ?>
			...
			<tr class="membership-note">
				<?php foreach ( $my_membership_notes_columns as $column_id => $column_name ) : ?>
					...
					<?php if ( has_action( 'wc_memberships_members_area_my_membership_notes_column_' . $column_id ) ) : ?>

						<?php do_action( 'wc_memberships_members_area_my_membership_notes_column_' . $column_id, $note ); ?>

					<?php elseif ( 'membership-note-date' == $column_id ) : ?>
