...
if ( ! function_exists( 'wc_memberships_get_members_area_url' ) ) {
	 ...
	 * @param string $members_area Optional, which section of the members area to point to
	 * @param int|string $paged Optional, for paged sections
	 * @return string Unescaped URL
	 */
	function wc_memberships_get_members_area_url( $membership, $members_area = '', $paged = '' ) {
		...
		if ( empty( $members_area ) ) {
			$membership_plan    = is_int( $membership ) ? wc_memberships_get_membership_plan( $membership ) : $membership;
			...
			$members_area       = key( $available_sections );
		}
		...
		if ( get_option( 'permalink_structure' ) ) {
			...
			$myaccount_url = SV_WC_Plugin_Compatibility::wc_get_page_permalink( 'myaccount' );
			$endpoint      = get_option( 'woocommerce_myaccount_mymembership_endpoint', 'members-area' );

			// e.g. /my-account/members-area/123/my-membership-content/2
			return $myaccount_url . $endpoint . '/' . $membership . '/' . $members_area . '/' . $paged;
		}
	...
	function wc_memberships_get_members_area_action_links( $section, $user_membership, $object ) {
		...
		switch ( $section ) {
			...
			case 'my-memberships' :
				...
				if ( ! $user_membership->is_cancelled() && ! $user_membership->is_expired() && current_user_can( 'wc_memberships_cancel_membership', $user_membership->get_id() ) ) {
				...
				if ( ! empty ( $members_area ) && is_array( $members_area ) ) {
					$default_actions['view'] = array(
						...
						'name' => __( 'View', WC_Memberships::TEXT_DOMAIN ),
					);
				}
		...
		// e.g. wc_memberships_members_area_my-membership-content_actions
		// -> wc_memberships_members_area_my_membership_content_actions
		...
		if ( $max_pages = $query->max_num_pages > 1 ) {
			...
			if ( $membership ) {
				...
				if ( 1 === $current_page ) {
					...
					$links .= '<a href="' . esc_url( wc_memberships_get_members_area_url( $membership, $section, 2 ) ). '" class="wc-memberships-members-area-page-link ' . $right . '">' . esc_html__( 'Next &rarr;', WC_Memberships::TEXT_DOMAIN ) . '</a>';
				} elseif ( $max_pages == $current_page ) {
					// Last page, show prev
					$links .= '<a href="' . esc_url( wc_memberships_get_members_area_url( $membership, $section, $current_page - 1 ) ) . '" class="wc-memberships-members-area-page-link ' . $left . '">' . esc_html__( '&larr; Previous', WC_Memberships::TEXT_DOMAIN ) . '</a>';
				} else {
					// In the middle of pages, show both
					$links .= '<a href="' . esc_url( wc_memberships_get_members_area_url( $membership, $section, $current_page - 1 ) ) . '" class="wc-memberships-members-area-page-link ' . $left . '">' . esc_html__( '&larr; Previous', WC_Memberships::TEXT_DOMAIN ) . '</a>';
					$links .= '<a href="' . esc_url( wc_memberships_get_members_area_url( $membership, $section, $current_page + 1 ) ) . '" class="wc-memberships-members-area-page-link ' . $right . '">' . esc_html__( 'Next &rarr;', WC_Memberships::TEXT_DOMAIN ) . '</a>';
				}
