...
class WC_Memberships_Integration_Subscriptions {
	...
	public function update_related_membership_status( $subscription, $user_membership, $status, $note = '' ) {
		...
		switch ( $status ) {
			...
			case 'active':
				...
				$trial_end = SV_WC_Plugin_Compatibility::is_wc_subscriptions_version_gte_2_0() && $subscription instanceof WC_Subscription
	...
	/**
	 * Handle subscription upgrades/downgrades (switch). Hook is availble in
	 * Subscriptions 2.0.6+ only.
	 *
	 * @since 1.3.8
	...
	public function handle_subscription_switches( $subscription, $new_order_item, $old_order_item ) {
		...
		foreach ( $user_memberships as $user_membership ) {
			...
			if ( absint( $old_product_id ) === absint( $user_membership->get_product_id() ) ) {
				...
				$note = __( 'Membership cancelled because subscription was switched.', WC_Memberships::TEXT_DOMAIN );
	...
	/**
	 * Only grant access to new subscriptions if they're not a subscription
	 * renewal
	 *
	 * @since 1.3.5
			...
			if ( $subscription instanceof WC_Subscription ) {
				...
				$actions = array_merge( array(
					'delete-with-subscription' => array(
						...
						'link'              => '#',
						'text'              => __( 'Delete User Membership with Subscription', WC_Memberships::TEXT_DOMAIN ),
						'custom_attributes' => array(
							'data-user-membership-id' => $user_membership_id,
							'data-subscription-id'    => $subscription->id,
							'data-tip'                => __( 'Delete this membership permanently and the subscription associated with it', WC_Memberships::TEXT_DOMAIN ),
						),
					),
				), $actions );
	...
	public function enable_discounts_to_sign_up_fees( $product_settings ) {
		...
		$new_option = array(
			array(
				...
				'name'     => __( 'Discounts apply to subscriptions sign up fees', WC_Memberships::TEXT_DOMAIN ),
				'desc'     => __( 'If enabled, membership discounts will also apply to sign up fees of subscription products.', WC_Memberships::TEXT_DOMAIN ),
				'default'  => 'no',
			),
		);
