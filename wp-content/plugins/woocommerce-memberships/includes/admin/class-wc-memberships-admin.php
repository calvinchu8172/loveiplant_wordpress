...
class WC_Memberships_Admin {
	...
	public function enqueue_scripts_and_styles( $hook_suffix ) {
		...
		wp_localize_script( 'wc-memberships-admin', 'wc_memberships_admin', array(
			...
			'i18n' => array(
				...
				'delete_membership_confirm' => __( 'Are you sure that you want to permanently delete this membership?', WC_Memberships::TEXT_DOMAIN ),
				'transfer_membership'       => __( 'Transfer Membership', WC_Memberships::TEXT_DOMAIN ),
	...
	public function render_order_data( $order ) {
			...
			echo '<p class="form-field form-field-wide wc-customer-memberships">';
			echo '<label for="customer_memberships">' . esc_html__( 'Active Memberships:', WC_Memberships::TEXT_DOMAIN ) . '</label>';
			...
			if ( ! empty( $memberships ) ) {
 			...
 			if ( empty( $memberships ) || ! $count ) {
				esc_html_e( 'none', WC_Memberships::TEXT_DOMAIN );
			}
	...
	public function add_user_columns( $columns ) {
		...
		if ( current_user_can( 'manage_woocommerce' ) ) {
			...
			array_pop( $columns );
			$columns['wc_memberships_user_memberships'] = __( 'Active Memberships', WC_Memberships::TEXT_DOMAIN );
			$columns += $last_column;
		}
	...
	public function show_user_memberships( WP_User $user ) {
		...
		if ( ! empty( $user_memberships ) ) {
			...
			if ( ! empty( $plan_links ) ) {
				printf( esc_html__( 'This user is a member of %s. %sAdd another membership%s.', WC_Memberships::TEXT_DOMAIN ), wc_memberships()->list_items( $plan_links, __( 'and', WC_Memberships::TEXT_DOMAIN ) ), '<a href="' . admin_url( 'post-new.php?post_type=wc_user_membership&user=' . $user->ID ) . '">', '</a>' );
			} else {
				_e( 'This user is already a member of every plan.', WC_Memberships::TEXT_DOMAIN );
			}
		}
		else {
