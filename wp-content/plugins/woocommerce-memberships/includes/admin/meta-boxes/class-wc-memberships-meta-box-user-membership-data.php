		...
		<div class="plan-details">
			...
			<div class="woocommerce_options_panel">
				...
				<?php woocommerce_wp_text_input( array(
					...
					'description' => __( 'YYYY-MM-DD', WC_Memberships::TEXT_DOMAIN ) . (
						empty( $membership_plan_options )
							? ''
							:  ' - <a href="#" class="js-calc-expiration">' . esc_html__( 'Update expiration date to plan length', WC_Memberships::TEXT_DOMAIN ) . '</a>'
						),
				...
				<?php if ( $paused_date = $user_membership->get_local_paused_date( 'timestamp' ) ) : ?>
					<p class="form-field"><span class="description"><?php printf( esc_html__( 'Paused since %s', WC_Memberships::TEXT_DOMAIN ), date_i18n( wc_date_format(), $paused_date ) ); ?></span></p>
				<?php endif; ?>
				...
				<?php
					...
					if ( current_user_can( 'delete_post', $post->ID ) ) {
						...
						$user_membership_actions['delete-action'] = array(
							...
							'text'  => __( 'Delete User Membership', WC_Memberships::TEXT_DOMAIN ),
						);
						...
						if ( 'auto-draft' != $post->post_status ) {
							...
							$user_membership_actions['transfer-action'] = array(
								...
								'link'              => '#',
								'text'              => __( 'Transfer', WC_Memberships::TEXT_DOMAIN ),
				...
				<?php if ( ! empty( $user_membership_actions ) ) : ?>
					...
					<?php foreach( $user_membership_actions as $id => $action ) : ?>
						<div id="<?php echo $id ?>" class="user-membership-action">
							...
							?>
							<a href="<?php echo esc_url( $action['link'] ); ?>" class="<?php echo esc_attr( $action['class'] ); ?>" <?php echo $custom_attr; ?>><?php echo esc_html( $action['text'] ); ?></a>
						</div>
					<?php endforeach; ?>

				<?php endif; ?>

				<input type="submit" class="button save_user_membership save_action button-primary tips" value="<?php esc_attr_e( 'Save', WC_Memberships::TEXT_DOMAIN ); ?>" data-tip="<?php esc_attr_e( 'Save/update the membership', WC_Memberships::TEXT_DOMAIN ); ?>" />
