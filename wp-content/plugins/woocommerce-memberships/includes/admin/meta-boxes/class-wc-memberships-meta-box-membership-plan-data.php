	...
	public function output( WP_Post $post ) {
		...
		<div class="panel-wrap data">
			...
			<ul class="membership_plan_data_tabs wc-tabs">
				<?php
					...
					$membership_plan_data_tabs = apply_filters( 'wc_membership_plan_data_tabs', array(
						...
						'members_area' => array(
							'label'  => __( 'Members Area', WC_Memberships::TEXT_DOMAIN ),
			...
			<div id="membership-plan-members-area" class="panel woocommerce_options_panel"><?php ?>
				...
				<p><?php esc_html_e( 'The Members Area is a portion of the My Accounts page that shows the member lists of available content, products, discounts, and / or membership notes.', WC_Memberships::TEXT_DOMAIN ); ?></p>

				<p class="form-field"><label for="_members_area_sections"><?php esc_html_e( 'Include sections for:', WC_Memberships::TEXT_DOMAIN ); ?></label>
					<?php
					...
					?>
					<select name="_members_area_sections[]" id="_members_area_sections" class="wc-enhanced-select-nostd" multiple="multiple" style="width: 50%;">
					...
					<img class="help_tip" data-tip="<?php esc_attr_e( 'Leave empty to hide members area for this membership.', WC_Memberships::TEXT_DOMAIN ) ?>" src="<?php echo esc_url( WC()->plugin_url() ); ?>/assets/images/help.png" height="16" width="16" />

				</p>

				<p><!-- // legend -->
					<?php if ( in_array( 'my-membership-content', array_keys( $members_area_sections ) ) ) : ?>
						<em><?php esc_html_e( '"My Content" will show all pages, posts and other content.', WC_Memberships::TEXT_DOMAIN ); ?></em><br>
					<?php endif; ?>
					<?php if ( in_array( 'my-membership-products', array_keys( $members_area_sections ) ) ) : ?>
						<em><?php esc_html_e( '"My Products" will show products that are viewable or purchaseable.', WC_Memberships::TEXT_DOMAIN ); ?></em><br>
					<?php endif; ?>
					<?php if ( in_array( 'my-membership-discounts', array_keys( $members_area_sections ) ) ) : ?>
						<em><?php esc_html_e( '"My Discounts" will list products carrying membership discounts.', WC_Memberships::TEXT_DOMAIN ); ?></em><br>
					<?php endif; ?>
					<?php if ( in_array( 'my-membership-notes', array_keys( $members_area_sections ) ) ) : ?>
						<em><?php esc_html_e( '"Membership Notes" will only display notes that have been emailed to the customer (no internal membership notes).', WC_Memberships::TEXT_DOMAIN ); ?></em>
					<?php endif; ?>
				</p>
