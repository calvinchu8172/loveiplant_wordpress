...
class WC_Memberships_Frontend {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_get_breadcrumb', array( $this, 'filter_breadcrumbs' ), 40 );
		add_filter( 'the_content',                array( $this, 'render_members_area_content' ), 40 );
	...
	public function filter_breadcrumbs( $crumbs ) {
		...
		if ( isset( $wp_query->query_vars['members_area'] ) && is_account_page( 'myaccount' ) && count( $crumbs ) > 0 ) {
		...
		if ( isset( $wp_query->query_vars['members_area'] ) && is_account_page() ) {
			...
			if ( $user_membership && $user_id == $user_membership->get_user_id() && wc_memberships_is_user_active_member( $user_id, $user_membership->get_plan() ) ) {
				...
				if ( ! empty( $members_area ) ) {
					...
					echo '<div id="wc-memberships-members-area" class="my-membership member-' . esc_attr( $user_id ) . '" data-member="' . esc_attr( $user_id ) . '" data-membership="' . esc_attr( $user_membership->get_plan()->get_id() ) . '">';
					...
					echo '<div id="wc-memberships-members-area-section" class="my-membership-section ' . sanitize_html_class( $section ). '" data-section="' . esc_attr( $section ).'"  data-page="1">';
	...
	public function get_member_login_message() {
		...
		if ( $message = get_option( 'wc_memberships_member_login_message' ) ) {
			 ...
			 */
			$message = __( 'Some items in you cart are discounted for members. %1$sLog in%2$s to claim them.', WC_Memberships::TEXT_DOMAIN );
			 ...
			 * Placeholders: %1$s - <a> tag, %2$s - </a> tag
			 */
			$message = __( 'An item in your cart is discounted for members. %1$sLog in%2$s to claim it.', WC_Memberships::TEXT_DOMAIN );
			 ...
			 * Placeholders: %1$s - <a> tag, %2$s - </a> tag
			 */
			$message = __( 'This item is discounted for members. %1$sLog in%2$s to claim it.', WC_Memberships::TEXT_DOMAIN );
		}
	...
	private function get_restricted_content_redirect_url() {
		...
		$url = SV_WC_Plugin_Compatibility::wc_get_page_permalink( 'myaccount' );
	...
	public function get_product_taxonomy_term_viewing_restricted_message( $taxonomy, $term_id ) {
		...
		if ( ! empty( $products ) ) {
			$text = sprintf(
				__( 'This product category can only be viewed by members. To view this category, sign up by purchasing %1$s, or %2$slog in%3$s if you are a member.', WC_Memberships::TEXT_DOMAIN ),
				'{products}',
				'<a href="' . esc_url( SV_WC_Plugin_Compatibility::wc_get_page_permalink( 'myaccount' ) ) .'">', '</a>'
			);
		} else {
			$text = __( 'This product category can only be viewed by members.', WC_Memberships::TEXT_DOMAIN );
		}
	...
	public function get_product_taxonomy_term_delayed_message( $user_id, $taxonomy, $term_id ) {
		...
		$post_id     = $this->get_post_id_from_taxonomy_term( $taxonomy, $term_id );
		$text        = __( 'This product category is part of your membership, but not yet! You will gain access on {date}', WC_Memberships::TEXT_DOMAIN );
