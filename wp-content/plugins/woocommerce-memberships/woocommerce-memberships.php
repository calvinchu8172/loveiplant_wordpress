...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.1.1', __( 'WooCommerce Memberships', 'woocommerce-memberships' ), __FILE__, 'init_woocommerce_memberships', array( 'minimum_wc_version' => '2.2', 'minimum_wp_version' => '3.9', 'backwards_compatible' => '4.0.0' ) );
...
class WC_Memberships extends SV_WC_Plugin {
	...
	public function get_members_area_template( $section, $args ) {
		...
		if ( 'my-membership-content' == $section ) {
			...
			$located = wc_memberships()->locate_template( 'myaccount/' . $section . '.php', '', '' );
			if ( is_readable( $located ) ) {
				wc_get_template( $section . '.php', $args );
			}
	...
	public function add_endpoints() {
		...
		add_rewrite_endpoint( $endpoint, EP_PAGES, false );
	...
	protected function upgrade( $installed_version ) {
		...
		if ( version_compare( $installed_version, '1.4.0', '<' ) ) {
			...
			update_option( 'wc_memberships_product_category_viewing_restricted_message', __( 'This product category can only be viewed by members. To view this category, sign up by purchasing {products}.', WC_Memberships::TEXT_DOMAIN ) );
			update_option( 'wc_memberships_product_category_viewing_restricted_message_no_products', __( 'Displays if viewing a product category is restricted to a membership that cannot be purchased.', WC_Memberships::TEXT_DOMAIN ) );
		}
