...
//        add_action('wp_head', array($this, 'add_fb_style_hide_comment_box'));
        ...
        add_action('wp_ajax_nopriv_rssocialinstagram', array($this, 'update_reward_points_for_instagram'));

        add_action('wp_ajax_rssocialinstagram', array($this, 'update_reward_points_for_instagram'));
        ...
        add_action('wp_ajax_nopriv_rssocialtwitterfollowcallback', array($this, 'update_reward_points_for_twitter_follow'));

        add_action('wp_ajax_rssocialtwitterfollowcallback', array($this, 'update_reward_points_for_twitter_follow'));
        ...
        add_shortcode('twitter_follow_reward_points', array($this, 'add_shortcode_for_social_twitter_follow'));
        ...
        add_shortcode('instagram_reward_points', array($this, 'add_shortcode_for_social_instagram'));
    }
    ...
    public static function reward_system_social_likes_buttons() {
        ...
        $wplang=get_option('WPLANG');
         $global_variable_for_js = array('wp_ajax_url' => admin_url('admin-ajax.php'), 'user_id' => get_current_user_id(),'product_title'=>$post_title,'url'=>get_permalink(),'caption'=>$product_caption,'images'=>$image[0],'product_id'=>$post->ID,'facebook_id'=>get_option('rs_facebook_application_id'),'language'=>$language,'wplang'=>$wplang);
    ...
    public static function add_fb_style_hide_comment_box() {
        ...
        if (get_post_meta(@$post->ID, '_socialrewardsystemcheckboxvalue', true) == 'yes') {
            ...
            <?php if (get_option('rs_reward_point_enable_tipsy_social_rewards') == '1') { ?>
                <?php if (get_option('rs_global_show_hide_social_tooltip_for_twitter_follow') == '1') { ?>
                    <script type="text/javascript">
                        jQuery(window).load(function () {
            ...
            <?php if (get_option('rs_reward_point_enable_tipsy_social_rewards') == '1') { ?>
                <?php if (get_option('rs_global_show_hide_social_tooltip_for_instagram') == '1') { ?>
                    <script type="text/javascript">
                        jQuery(window).load(function () {
        ...
        if ($banning_type != 'earningonly' && $banning_type != 'both') {
            ...
            if (isset($_POST['state']) && ($_POST['postid']) && ($_POST['currentuserid'])) {
                if (get_option('rs_set_price_to_calculate_rewardpoints_by_percentage') == '1') {
                 ...
                 $getregularprice = RSFunctionForCart::get_discount_price($_POST['postid']);
                $postid = $_POST['postid'];
            ...
            if (isset($_POST['state']) && ($_POST['postid']) && ($_POST['currentuserid'])) {
                ...
                if (!empty($oldoption)) {
                    if (!in_array($_POST['postid'], $oldoption)) {
                        ...
                        if ($_POST['state'] == 'on') {
                            $checklevel = self::checklevel_for_instagram_follow($postid);
                            $orderid = '0';
                            $totalearnedpoints = '0';
                            $totalredeempoints = '0';
                            $pointsredeemed = '0';
                            $reasonindetail = '';
                            self::rs_insert_instagram_share_points($pointsredeemed, $getregularprice, $postid, $checklevel, $currentuserid, $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail);
                        }
                    } else {
    ...
    public static function checklevel_for_instagram_follow($postid) {

//Product Level
        $enablerewards = RSFunctionForSavingMetaValues::rewardsystem_get_post_meta($postid, '_socialrewardsystemcheckboxvalue');
        $gettype = RSFunctionForSavingMetaValues::rewardsystem_get_post_meta($postid, '_social_rewardsystem_options_instagram');
        $getpoints = RSFunctionForSavingMetaValues::rewardsystem_get_post_meta($postid, '_socialrewardsystempoints_instagram');
        $getpercent = RSFunctionForSavingMetaValues::rewardsystem_get_post_meta($postid, '_socialrewardsystempercent_instagram');
...
//Global Level
    ...
    public static function rs_insert_instagram_share_points($pointsredeemed, $getregularprice, $postid, $level, $currentuserid, $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail) {
...
//Global Level Points and Percent
                        ...
                        if (($restrictuserpoints != '') && ($restrictuserpoints != '0')) {
                            ...
                            if ($getoldpoints <= $restrictuserpoints) {
                                ...
                                if ($totalpointss <= $restrictuserpoints) {
                                    ...
                                    RSPointExpiry::insert_earning_points($currentuserid, $productlevelrewardpointss, $pointsredeemed, $date, 'RPIF', $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail);
                                    ...
                                    RSPointExpiry::record_the_points($currentuserid, $productlevelrewardpointss, $pointsredeemed, $date, 'RPIF', $equearnamt, $equredeemamt, $order_id, $postid, $variationid, $refuserid, '', $totalpoints, '', '0');
                                } else {
                        ...
                        if (($restrictuserpoints != '') && ($restrictuserpoints != '0')) {
                            ...
                            if ($getoldpoints <= $restrictuserpoints) {
                                ...
                                if ($totalpointss <= $restrictuserpoints) {
                                    ...
                                    RSPointExpiry::insert_earning_points($currentuserid, $productlevelrewardpercentss, $pointsredeemed, $date, 'RPIF', $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail);
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($productlevelrewardpercentss);
                                    $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($currentuserid);
                                    RSPointExpiry::record_the_points($currentuserid, $productlevelrewardpercentss, $pointsredeemed, $date, 'RPIF', $equearnamt, $equredeemamt, $order_id, $postid, $variationid, $refuserid, '', $totalpoints, '', '0');
                                } else {
                                    $insertpoints = $restrictuserpoints - $getoldpoints;
                                    RSPointExpiry::insert_earning_points($currentuserid, $insertpoints, $pointsredeemed, $date, 'MREPFU', $order_id, $totalearnedpoints, $totalredeempoints, '');
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($insertpoints);
                        ...
                        if (($restrictuserpoints != '') && ($restrictuserpoints != '0')) {
                            ...
                            if ($getoldpoints <= $restrictuserpoints) {
                                ...
                                if ($totalpointss <= $restrictuserpoints) {
                                    ...
                                    RSPointExpiry::insert_earning_points($currentuserid, $categorylevelrewardpointss, $pointsredeemed, $date, 'RPIF', $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail);
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($categorylevelrewardpointss);
                                    $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($currentuserid);
                                    RSPointExpiry::record_the_points($currentuserid, $categorylevelrewardpointss, $pointsredeemed, $date, 'RPIF', $equearnamt, $equredeemamt, $order_id, $postid, $variationid, $refuserid, '', $totalpoints, '', '0');
                                } else {
                                    $insertpoints = $restrictuserpoints - $getoldpoints;
                                    RSPointExpiry::insert_earning_points($currentuserid, $insertpoints, $pointsredeemed, $date, 'MREPFU', $order_id, $totalearnedpoints, $totalredeempoints, '');
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($insertpoints);
                        ...
                        if (($restrictuserpoints != '') && ($restrictuserpoints != '0')) {
                            ...
                            if ($getoldpoints <= $restrictuserpoints) {
                                ...
                                if ($totalpointss <= $restrictuserpoints) {
                                    ...
                                    RSPointExpiry::insert_earning_points($currentuserid, $categorylevelrewardpercents, $pointsredeemed, $date, 'RPIF', $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail);
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($categorylevelrewardpercents);
                                    $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($currentuserid);
                                    RSPointExpiry::record_the_points($currentuserid, $categorylevelrewardpercents, $pointsredeemed, $date, 'RPIF', $equearnamt, $equredeemamt, $order_id, $postid, $variationid, $refuserid, '', $totalpoints, '', '0');
                                } else {
                                    $insertpoints = $restrictuserpoints - $getoldpoints;
                                    RSPointExpiry::insert_earning_points($currentuserid, $insertpoints, $pointsredeemed, $date, 'MREPFU', $order_id, $totalearnedpoints, $totalredeempoints, '');
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($insertpoints);
                        ...
                        if (($restrictuserpoints != '') && ($restrictuserpoints != '0')) {
                            ...
                            if ($getoldpoints <= $restrictuserpoints) {
                                ...
                                if ($totalpointss <= $restrictuserpoints) {
                                    ...
                                    RSPointExpiry::insert_earning_points($currentuserid, $global_rewardpointss, $pointsredeemed, $date, 'RPIF', $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail);
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($global_rewardpointss);
                                    $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($currentuserid);
                                    RSPointExpiry::record_the_points($currentuserid, $global_rewardpointss, $pointsredeemed, $date, 'RPIF', $equearnamt, $equredeemamt, $order_id, $postid, $variationid, $refuserid, '', $totalpoints, '', '0');
                                } else {
                                    $insertpoints = $restrictuserpoints - $getoldpoints;
                                    RSPointExpiry::insert_earning_points($currentuserid, $insertpoints, $pointsredeemed, $date, 'MREPFU', $order_id, $totalearnedpoints, $totalredeempoints, '');
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($insertpoints);
                        ...
                        if (($restrictuserpoints != '') && ($restrictuserpoints != '0')) {
                            ...
                            if ($getoldpoints <= $restrictuserpoints) {
                                ...
                                if ($totalpointss <= $restrictuserpoints) {
                                    ...
                                    RSPointExpiry::insert_earning_points($currentuserid, $global_rewardpercents, $pointsredeemed, $date, 'RPIF', $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail);
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($global_rewardpercents);
                                    $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($currentuserid);
                                    RSPointExpiry::record_the_points($currentuserid, $global_rewardpercents, $pointsredeemed, $date, 'RPIF', $equearnamt, $equredeemamt, $order_id, $postid, $variationid, $refuserid, '', $totalpoints, '', '0');
                                } else {
                                    $insertpoints = $restrictuserpoints - $getoldpoints;
                                    RSPointExpiry::insert_earning_points($currentuserid, $insertpoints, $pointsredeemed, $date, 'MREPFU', $order_id, $totalearnedpoints, $totalredeempoints, '');
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($insertpoints);
    ...
    public static function checklevel_for_twitter_follow($postid) {

//Product Level
        $enablerewards = RSFunctionForSavingMetaValues::rewardsystem_get_post_meta($postid, '_socialrewardsystemcheckboxvalue');
        $gettype = RSFunctionForSavingMetaValues::rewardsystem_get_post_meta($postid, '_social_rewardsystem_options_twitter_follow');
        $getpoints = RSFunctionForSavingMetaValues::rewardsystem_get_post_meta($postid, '_socialrewardsystempoints_twitter_follow');
        $getpercent = RSFunctionForSavingMetaValues::rewardsystem_get_post_meta($postid, '_socialrewardsystempercent_twitter_follow');
...
//Global Level
    ...
    public static function rs_insert_twitter_follow_points($pointsredeemed, $getregularprice, $postid, $level, $currentuserid, $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail) {
...
//Global Level Points and Percent
                    ...
                    if ($enabledisablemaxpoints == 'yes') {
                        if (($restrictuserpoints != '') && ($restrictuserpoints != '0')) {
                            ...
                            if ($getoldpoints <= $restrictuserpoints) {
                                ...
                                if ($totalpointss <= $restrictuserpoints) {
                                    $productlevelrewardpointss = RSMemberFunction::user_role_based_reward_points($currentuserid, $getpoints);
                                    RSPointExpiry::insert_earning_points($currentuserid, $productlevelrewardpointss, $pointsredeemed, $date, 'RPTF', $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail);
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($productlevelrewardpointss);
                                    $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($currentuserid);
                                    RSPointExpiry::record_the_points($currentuserid, $productlevelrewardpointss, $pointsredeemed, $date, 'RPTF', $equearnamt, $equredeemamt, $order_id, $productid, $variationid, $refuserid, '', $totalpoints, '', '0');
                                } else {
                                    $insertpoints = $restrictuserpoints - $getoldpoints;
                                    RSPointExpiry::insert_earning_points($currentuserid, $insertpoints, $pointsredeemed, $date, 'MREPFU', $order_id, $totalearnedpoints, $totalredeempoints, '');
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($insertpoints);
                    ...
                    if ($enabledisablemaxpoints == 'yes') {
                        if (($restrictuserpoints != '') && ($restrictuserpoints != '0')) {
                            ...
                            if ($getoldpoints <= $restrictuserpoints) {
                                ...
                                if ($totalpointss <= $restrictuserpoints) {
                                    $productlevelrewardpercentss = RSMemberFunction::user_role_based_reward_points($currentuserid, $rewardpercentsforproductlevel);
                                    RSPointExpiry::insert_earning_points($currentuserid, $productlevelrewardpercentss, $pointsredeemed, $date, 'RPTF', $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail);
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($productlevelrewardpercentss);
                                    $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($currentuserid);
                                    RSPointExpiry::record_the_points($currentuserid, $productlevelrewardpercentss, $pointsredeemed, $date, 'RPTF', $equearnamt, $equredeemamt, $order_id, $productid, $variationid, $refuserid, '', $totalpoints, '', '0');
                                } else {
                                    $insertpoints = $restrictuserpoints - $getoldpoints;
                                    RSPointExpiry::insert_earning_points($currentuserid, $insertpoints, $pointsredeemed, $date, 'MREPFU', $order_id, $totalearnedpoints, $totalredeempoints, '');
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($insertpoints);
                    ...
                    if ($enabledisablemaxpoints == 'yes') {
                        if (($restrictuserpoints != '') && ($restrictuserpoints != '0')) {
                            ...
                            if ($getoldpoints <= $restrictuserpoints) {
                                ...
                                if ($totalpointss <= $restrictuserpoints) {
                                    $categorylevelrewardpointss = RSMemberFunction::user_role_based_reward_points($currentuserid, $getcategorypoints);
                                    RSPointExpiry::insert_earning_points($currentuserid, $categorylevelrewardpointss, $pointsredeemed, $date, 'RPTF', $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail);
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($categorylevelrewardpointss);
                                    $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($currentuserid);
                                    RSPointExpiry::record_the_points($currentuserid, $categorylevelrewardpointss, $pointsredeemed, $date, 'RPTF', $equearnamt, $equredeemamt, $order_id, $productid, $variationid, $refuserid, '', $totalpoints, '', '0');
                                } else {
                                    $insertpoints = $restrictuserpoints - $getoldpoints;
                                    RSPointExpiry::insert_earning_points($currentuserid, $insertpoints, $pointsredeemed, $date, 'MREPFU', $order_id, $totalearnedpoints, $totalredeempoints, '');
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($insertpoints);
                    ...
                    if ($enabledisablemaxpoints == 'yes') {
                        if (($restrictuserpoints != '') && ($restrictuserpoints != '0')) {
                            ...
                            if ($getoldpoints <= $restrictuserpoints) {
                                ...
                                if ($totalpointss <= $restrictuserpoints) {
                                    $categorylevelrewardpercents = RSMemberFunction::user_role_based_reward_points($currentuserid, $getcategorypercent);
                                    RSPointExpiry::insert_earning_points($currentuserid, $categorylevelrewardpercents, $pointsredeemed, $date, 'RPTF', $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail);
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($categorylevelrewardpercents);
                                    $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($currentuserid);
                                    RSPointExpiry::record_the_points($currentuserid, $categorylevelrewardpercents, $pointsredeemed, $date, 'RPTF', $equearnamt, $equredeemamt, $order_id, $productid, $variationid, $refuserid, '', $totalpoints, '', '0');
                                } else {
                                    $insertpoints = $restrictuserpoints - $getoldpoints;
                                    RSPointExpiry::insert_earning_points($currentuserid, $insertpoints, $pointsredeemed, $date, 'MREPFU', $order_id, $totalearnedpoints, $totalredeempoints, '');
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($insertpoints);
                    ...
                    if ($enabledisablemaxpoints == 'yes') {
                        if (($restrictuserpoints != '') && ($restrictuserpoints != '0')) {
                            ...
                            if ($getoldpoints <= $restrictuserpoints) {
                                ...
                                if ($totalpointss <= $restrictuserpoints) {
                                    $global_rewardpointss = RSMemberFunction::user_role_based_reward_points($currentuserid, $global_reward_points);
                                    RSPointExpiry::insert_earning_points($currentuserid, $global_rewardpointss, $pointsredeemed, $date, 'RPTF', $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail);
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($global_rewardpointss);
                                    $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($currentuserid);
                                    RSPointExpiry::record_the_points($currentuserid, $global_rewardpointss, $pointsredeemed, $date, 'RPTF', $equearnamt, $equredeemamt, $order_id, $productid, $variationid, $refuserid, '', $totalpoints, '', '0');
                                } else {
                                    $insertpoints = $restrictuserpoints - $getoldpoints;
                                    RSPointExpiry::insert_earning_points($currentuserid, $insertpoints, $pointsredeemed, $date, 'MREPFU', $order_id, $totalearnedpoints, $totalredeempoints, '');
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($insertpoints);
                    ...
                    if ($enabledisablemaxpoints == 'yes') {
                        if (($restrictuserpoints != '') && ($restrictuserpoints != '0')) {
                            ...
                            if ($getoldpoints <= $restrictuserpoints) {
                                ...
                                if ($totalpointss <= $restrictuserpoints) {
                                    $global_rewardpercents = RSMemberFunction::user_role_based_reward_points($currentuserid, $rewardpercentsforgloballevel);
                                    RSPointExpiry::insert_earning_points($currentuserid, $global_rewardpercents, $pointsredeemed, $date, 'RPTF', $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail);
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($global_rewardpercents);
                                    $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($currentuserid);
                                    RSPointExpiry::record_the_points($currentuserid, $global_rewardpercents, $pointsredeemed, $date, 'RPTF', $equearnamt, $equredeemamt, $order_id, $productid, $variationid, $refuserid, '', $totalpoints, '', '0');
                                } else {
                                    $insertpoints = $restrictuserpoints - $getoldpoints;
                                    RSPointExpiry::insert_earning_points($currentuserid, $insertpoints, $pointsredeemed, $date, 'MREPFU', $order_id, $totalearnedpoints, $totalredeempoints, '');
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($insertpoints);
    ...
    public static function add_shortcode_for_social_instagram($contents) {
        ...
        $getaction = get_post_meta($post->ID, '_social_rewardsystem_options_instagram', true);
        $getpoints = get_post_meta($post->ID, '_socialrewardsystempoints_instagram', true);
        $getpercent = get_post_meta($post->ID, '_socialrewardsystempercent_instagram', true);
        if (get_option('rs_set_price_to_calculate_rewardpoints_by_percentage') == '1') {
          ...
          $getregularprice = RSFunctionForCart::get_discount_price($post->ID);
        if ($enablerewards == 'yes') {
            if ($getaction == '1') {
                ...
                if ($getpoints == '') {
                    ...
                    if (is_array($term)) {
                        ...
                        foreach ($term as $term) {
                            ...
                            $display_type = get_woocommerce_term_meta($term->term_id, 'social_instagram_enable_rs_rule', true);
                            if (($enablevalue == 'yes') && ( $enablevalue != '')) {
                                if ($display_type == '1') {
                                    if (get_woocommerce_term_meta($term->term_id, 'social_instagram_rs_category_points', true) == '') {
                                        if ($global_enable == '1') {
                                            if ($global_reward_type == '1') {
                                                $rewardpoints[] = get_option('rs_global_social_instagram_reward_points');
                                            } else {
                                                $pointconversion = get_option('rs_earn_point');
                                                $pointconversionvalue = get_option('rs_earn_point_value');
                                        ...
                                        $rewardpoints[] = get_woocommerce_term_meta($term->term_id, 'social_instagram_rs_category_points', true);
                                    }
                                } else {
                                    $pointconversion = get_option('rs_earn_point');
                                    $pointconversionvalue = get_option('rs_earn_point_value');
                                    $getaverage = get_woocommerce_term_meta($term->term_id, 'social_instagram_rs_category_percent', true) / 100;
                                    $getaveragepoints = $getaverage * $getregularprice;
                                    $pointswithvalue = $getaveragepoints * $pointconversion;
                                    if (get_woocommerce_term_meta($term->term_id, 'social_instagram_rs_category_percent', true) == '') {
                                        ...
                                        if ($global_enable == '1') {
                                            if ($global_reward_type == '1') {
                                                ...
                                                $pointconversion = get_option('rs_earn_point');
                                                $pointconversionvalue = get_option('rs_earn_point_value');
                                                $getaverage = get_option('rs_global_social_instagram_reward_percent') / 100;
                                                $getaveragepoints = $getaverage * $getregularprice;
                                                $pointswithvalue = $getaveragepoints * $pointconversion;
                                                $rewardpoints[] = $pointswithvalue / $pointconversionvalue;
                                            }
            ...
            if (isset($_POST['state']) && ($_POST['postid']) && ($_POST['currentuserid'])) {
                ...
                if (!empty($oldoption)) {
                    if (!in_array($_POST['postid'], $oldoption)) {
                        ...
                        if ($_POST['state'] == 'on') {
                            $checklevel = self::checklevel_for_twitter_follow($postid);
                            $orderid = '0';
                            $totalearnedpoints = '0';
                            $totalredeempoints = '0';
                            $pointsredeemed = '0';
                            $reasonindetail = '';
                            self::rs_insert_twitter_follow_points($pointsredeemed, $getregularprice, $postid, $checklevel, $currentuserid, $orderid, $totalearnedpoints, $totalredeempoints, $reasonindetail);
                        }
                    } else {
...
//Global Level Points and Percent
    ...
    public static function add_shortcode_for_social_twitter_follow() {
        ...
        $global_reward_type = get_option('rs_global_social_reward_type_twitter_follow');
        $enablerewards = get_post_meta($post->ID, '_socialrewardsystemcheckboxvalue', true);
        $getaction = get_post_meta($post->ID, '_social_rewardsystem_options_twitter_follow', true);
        $getpoints = get_post_meta($post->ID, '_socialrewardsystempoints_twitter_follow', true);
        $getpercent = get_post_meta($post->ID, '_socialrewardsystempercent_twitter_follow', true);
        if (get_option('rs_set_price_to_calculate_rewardpoints_by_percentage') == '1') {
            $getregularprice = get_post_meta($post->ID, '_regular_price', true);
            if ($getregularprice == '') {
                $getregularprice = get_post_meta($post->ID, '_price', true);
            ...
            if ($getaction == '1') {
                if ($getpoints == '') {
                    ...
                    if (is_array($term)) {
                        foreach ($term as $term) {
                            $enablevalue = get_woocommerce_term_meta($term->term_id, 'enable_social_reward_system_category', true);
                            $display_type = get_woocommerce_term_meta($term->term_id, 'social_twitter_follow_enable_rs_rule', true);
                            if (($enablevalue == 'yes') && ( $enablevalue != '')) {
                                if ($display_type == '1') {
                                    if (get_woocommerce_term_meta($term->term_id, 'social_twitter_follow_rs_category_points', true) == '') {
                                        if ($global_enable == '1') {
                                            if ($global_reward_type == '1') {
                                                $rewardpoints[] = get_option('rs_global_social_twitter_follow_reward_points');
                                            } else {
                                                $pointconversion = get_option('rs_earn_point');
                                                $pointconversionvalue = get_option('rs_earn_point_value');
                                                $getaverage = get_option('rs_global_social_twitter_follow_reward_percent') / 100;
                                                $getaveragepoints = $getaverage * $getregularprice;
                                                $pointswithvalue = $getaveragepoints * $pointconversion;
                                                $rewardpoints[] = $pointswithvalue / $pointconversionvalue;
                                            }
                                        ...
                                        $rewardpoints[] = get_woocommerce_term_meta($term->term_id, 'social_twitter_follow_rs_category_points', true);
                                    }
                                } else {
                                    $pointconversion = get_option('rs_earn_point');
                                    $pointconversionvalue = get_option('rs_earn_point_value');
                                    $getaverage = get_woocommerce_term_meta($term->term_id, 'social_twitter_follow_rs_category_percent', true) / 100;
                                    $getaveragepoints = $getaverage * $getregularprice;
                                    $pointswithvalue = $getaveragepoints * $pointconversion;
                                    if (get_woocommerce_term_meta($term->term_id, 'social_twitter_follow_rs_category_percent', true) == '') {
                                        if ($global_enable == '1') {
                                            if ($global_reward_type == '1') {
                                                $rewardpoints[] = get_option('rs_global_social_twitter_follow_reward_points');
                                            } else {
