...
class RSFunctionForCart {
    ...
    public function __construct() {
        ...
        if (get_option('rs_show_hide_redeem_field') == '1') {
            if (get_option('rs_reward_point_troubleshoot_after_cart') == '1') {
                add_action('woocommerce_after_cart_table', array($this, 'reward_system_add_redeem_message_after_cart_table'));
            } else {
                add_action('woocommerce_cart_coupon', array($this, 'reward_system_add_redeem_message_after_cart_table'));
            }
        }
        ...
        add_action('woocommerce_before_cart', array($this, 'display_redeem_min_max_points_buttons_on_cart_page'));
        ...
        add_filter('woocommerce_add_to_cart_validation', array($this, 'sell_individually_functionality'), 10, 5);
          ...
          add_filter('vartable_add_to_cart_validation', array($this, 'sell_individually_functionality'), 10, 5);//compatability with woo-variations-table plugin
        ...
        add_action('admin_head', array($this, 'validate_maximum_minimum'), 10);
    }
    ...
    public static function get_product_price_in_cart() {
        ...
        foreach ($woocommerce->cart->cart_contents as $key => $value) {
            ...
            $user_ID = get_current_user_id();
            $getregularprice = $value['data']->get_price();
            ...
            if (is_object($checkproduct) && ($checkproduct->is_type('simple') || ($checkproduct->is_type('subscription')) || ($checkproduct->is_type('booking')))) {
                ...
                if ($value['variation_id'] != 0 && $value['variation_id'] != '') {
                    if (wp_get_post_parent_id($value['variation_id']) != '0') {
                        $variable_product1 = new WC_Product_Variation($value['variation_id']);
                    } else {
                        $variable_product1 = get_product($value['variation_id']);
                    }
                    if (get_option('rs_set_price_to_calculate_rewardpoints_by_percentage') == '1') {
                    ...
                    $variationregularprice = $value['data']->get_price();
                    if ($checkenablevariation == '1') {
                        if ($variationselectrule == '1') {
                            if (wp_get_post_parent_id($value['variation_id']) != '0') {
                                ...
                                $newparentid = $value['variation_id'];
                            }
            ...
            if ($_GET['tab'] == 'rewardsystem_cart') {
                ...
                if ((float) $woocommerce->version <= (float) ('2.2.0')) {
                    ...
                    echo RSJQueryFunction::rs_common_chosen_function('#rs_exclude_category_to_enable_redeeming');
                     echo RSJQueryFunction::rs_common_chosen_function('#rs_select_category_for_purchase_using_points');
                } else {
                    ...
                    echo RSJQueryFunction::rs_common_select_function('#rs_select_category_for_purchase_using_points');
                }
                ?>
                <script type="text/javascript">
                    jQuery(document).ready(function () {
                    ...
                    if ($current_carttotal_amount >= $minimum_cart_total_redeem && $current_carttotal_amount <= $maximum_cart_total_redeem) {
                        ...
                        if (get_option('rs_show_hide_maximum_cart_total_error_message') == '1') {
                            ...
                            if ($banning_type != 'redeemingonly' && $banning_type != 'both') {
                                ...
                                $max_cart_total_currency_to_replace = self::get_woocommerce_formatted_price($max_cart_amount_to_replace);
    ...
    public static function get_discount_price($product_id) {
        $product = new WC_Product($product_id);
