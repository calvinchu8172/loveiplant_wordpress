 ...
 * Main Class
 */
if (!class_exists('RightPress_Conditions_Helper_10')) {

final class RightPress_Conditions_Helper_10
{
        ...
        foreach ($posts_raw as $post_id) {
            ...
            if ($parent_id = wp_get_post_parent_id($post_id)) {
                if (RightPress_Helper::post_exists($parent_id)) {
                    ...
                    $items[] = array(
                        'id'    => (string) $post_id,
                        'text'  => sprintf(__('#%1$s of', 'rp_wcdpd'), $post_id) . ' ' . get_the_title($parent_id) . ' (' . $attributes . ')',
                    );
                }
            }
        }
