...
class RP_WCDPD_Controller_Methods_Cart_Discount extends RP_WCDPD_Controller_Methods
{
    ...
    protected $remove_processed = false;
    ...
    public function __construct()
    {
        ...
        add_action('woocommerce_cart_loaded_from_session', array($this, 'prepare_cart_discounts'));
        add_action('woocommerce_applied_coupon', array($this, 'prepare_cart_discounts'));

        // Remove no longer applicable cart discounts
        add_action('woocommerce_check_cart_items', array($this, 'remove_cart_discounts'), 1);
        add_action('woocommerce_before_calculate_totals', array($this, 'remove_cart_discounts'), 1);
    ...
    public function prepare_cart_discounts()
    {
        ...
        // Continue later
        if (!empty($this->applicable_adjustments)) {
        ...
        if (!RP_WCDPD_Settings::get('cart_discounts_apply_with_individual_use_coupons')) {
            ...
            if (is_array($woocommerce->cart->applied_coupons)) {
                foreach ($woocommerce->cart->applied_coupons as $coupon_code) {
                    ...
                    // Skip cart discounts
                    if ($this->coupon_is_cart_discount($coupon_code)) {
                        continue;
                    }
    ...
    public function remove_cart_discounts()
    {
        ...
        if ($this->remove_processed) {
            return;
        }
        $this->remove_processed = true;
        ...
        foreach ($woocommerce->cart->applied_coupons as $applied_coupon) {
            ...
            if ($this->coupon_is_cart_discount($applied_coupon)) {
     ...
     * @return bool
     */
    public function coupon_is_cart_discount($coupon_code)
    {
