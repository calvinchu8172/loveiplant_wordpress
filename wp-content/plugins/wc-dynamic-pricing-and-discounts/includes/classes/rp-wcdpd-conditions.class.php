...
class RP_WCDPD_Conditions
{
     ...
     * @return void
     */
    public static function display_fields($context, $combined_condition_key, $position)
    {
        if ($condition = RP_WCDPD_Controller_Conditions::get_item($combined_condition_key)) {
            if (isset($condition['fields']) && is_array($condition['fields']) && !empty($condition['fields'][$position])) {
                ...
                foreach ($condition['fields'][$position] as $field_key) {
                    if ($field = RP_WCDPD_Controller_Condition_Fields::get_item($field_key)) {
                        ...
                        // Open container
                        echo '<div class="rp_wcdpd_condition_setting_fields_' . $field_size . '">';

                        // Print field
                        call_user_func($field['display_callback'], $context);
