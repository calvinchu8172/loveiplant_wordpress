...
class RP_WCDPD_WC_Cart
{
     ...
     *
     * Note: PHP sorting methods are unstable, that is if two items are considered
     * equal their sort order is not defined. It may happen that two cart items
     * with the same prices are reordered. If this will cause trouble, we can
     * change uasort to our own sorting method.
     *
     * @access public
    ...
    public static function sort_cart_items_by_price($cart_items = null, $sort_order = 'ascending')
    {
        ...
        uasort($cart_items, array('RP_WCDPD_WC_Cart', $sort_comparison_method));
    ...
    public static function calculate_subtotal()
    {
        ...
        foreach ($woocommerce->cart->get_cart() as $cart_item) {
            if (isset($cart_item['line_subtotal']) && isset($cart_item['line_subtotal_tax'])) {
