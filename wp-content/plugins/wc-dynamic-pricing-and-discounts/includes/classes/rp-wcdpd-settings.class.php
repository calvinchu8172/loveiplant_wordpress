...
class RP_WCDPD_Settings
{
    ...
    public static function validate_rules($rule_type, $input)
    {
        ...
        if (!empty($input[$rule_type]) && is_array($input[$rule_type])) {
            ...
            foreach ($input[$rule_type] as $posted) {
                ...
                // Conditions
                $current['conditions'] = RP_WCDPD_Settings::validate_conditions($rule_type, $current, $posted);
    ...
    public static function validate_product_pricing_rule($current, $posted)
    {
        ...
        if (in_array($current['method'], array('bogo', 'bogo_repeat'), true)) {
            ...
            // Receive products type
            if (isset($posted['bogo_receive_products']) && RP_WCDPD_Settings::receive_products_method_exists($posted['bogo_receive_products'])) {
                $current['bogo_receive_products'] = $posted['bogo_receive_products'];
            }
            else {
                return false;
            }
            ...
            // Multiselect items
            foreach (array('products', 'product_variations', 'product_categories', 'product_attributes', 'product_tags') as $multiselect_field) {
                if (isset($posted['bogo_' . $multiselect_field]) && !RightPress_Helper::is_empty($posted['bogo_' . $multiselect_field])) {
                    $current['bogo_' . $multiselect_field] = (array) $posted['bogo_' . $multiselect_field];
                }
                else {
                    $current['bogo_' . $multiselect_field] = array();
                }
            }
        }
     ...
     * @return array
     */
    public static function validate_conditions($rule_type, $current, $posted)
    {
        $conditions = array();

        // Iterate over posted conditions
        if (!empty($posted['conditions']) && is_array($posted['conditions'])) {
            foreach ($posted['conditions'] as $condition) {
    ...
    public static function validate_single_quantity_range($quantity_range)
    {
        ...
        // Pricing method
        if (isset($quantity_range['pricing_method']) && RP_WCDPD_Pricing::pricing_method_exists($quantity_range['pricing_method'], 'product_pricing_quantity')) {
    ...
    public function configuration_to_javascript()
    {
        ...
        wp_localize_script('rp-wcdpd-rules-scripts', 'rp_wcdpd_multiselect_options', $this->get_multiselect_option_labels($current_tab));
    }
     ...
     * @return array
     */
    public function get_multiselect_option_labels($context)
    {
        $labels = array();

        // Iterate over rules of current context
        if (in_array($context, self::$contexts, true) && !empty($this->settings[$context])) {
            foreach ($this->settings[$context] as $row_key => $row) {

                // Conditions and group products
                foreach (array('conditions', 'group_products') as $child_type) {
            ...
            self::$receive_products_methods = array(
                'same' => array(
                    ...
                    'options' => array(
                        ...
                        'matched' => __('Same as products to purchase', 'rp_wcdpd'),
                    ),
                ),
                'products' => array(
