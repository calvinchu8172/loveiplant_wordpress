...
jQuery(document).ready(function() {
    ...
    function validate_product_pricing(key, form, panel)
    {
        ...
        // Exclude rules must have at least one condition
        if (method === 'exclude') {
            if (!panel.find('.rp_wcdpd_condition').length) {
                set_error(key, form, panel, panel.find('.rp_wcdpd_row_content_conditions_row label').filter(function() { return jQuery(this).css('display') === 'block'; }), rp_wcdpd.error_messages.no_conditions);
                is_valid = false;
            }
        }
