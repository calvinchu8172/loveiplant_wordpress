...
jQuery(document).ready(function() {
    ...
    function set_up_parent(container, key, config)
    {
        ...
        // At least one row exists
            ...
            jQuery('#rp_wcdpd_' + key + ' #rp_wcdpd_rule_wrapper .rp_wcdpd_row').each(function() {
                ...
                row.find('.rp_wcdpd_condition').each(function() {

                    // Fields
                    fix_condition(key, jQuery(this));

                    // Field identifiers
                    fix_child_element(key, 'conditions', jQuery(this), i, j);

                    // Field values
                    fix_child_values(key, false, 'condition', row, i, jQuery(this), j);

                    // Fix meta and coupon conditions (correct values were not present when it was first run)
                    toggle_condition_fields(key, jQuery(this), jQuery(this).find('.rp_wcdpd_' + key + '_condition_type').val());
    ...
    function duplicate_row(key, row)
    {
        ...
        row.find('.rp_wcdpd_condition').each(function() {
            ...
            fix_child_values(key, false, 'condition', row, row_key, jQuery(this), jQuery(this).index(), top_level_config, top_level_multiselect_options);
    ...
    function add(key, type, row, row_key, config)
    {
        ...
        if (config === false) {
            ...
            if (type === 'condition') {
                ...
                fix_child_element(key, 'conditions', child_row, row_key, child_row_key);

                // Fix condition field values
                fix_child_values(key, true, 'condition', row, row_key, child_row, child_row_key);
            }
    ...
    function fix_condition(key, element)
    {
        ...
        var condition_type = element.find('.rp_wcdpd_' + key + '_condition_type').val();

        // Condition type
        element.find('.rp_wcdpd_' + key + '_condition_type').change(function() {
            condition_type = element.find('.rp_wcdpd_' + key + '_condition_type').val();
            toggle_condition_fields(key, element, condition_type);
        });
        toggle_condition_fields(key, element, condition_type);
    }
     ...
     */
    function toggle_condition_fields(key, element, condition_type)
    {
        // Reference wrapper and row
        var wrapper = element.find('.rp_wcdpd_condition_setting_fields_wrapper');
        var row = element.closest('.rp_wcdpd_row');

        // Make sure we don't have required set of fields yet
        if (!wrapper.find('.rp_wcdpd_condition_setting_fields_' + condition_type).length) {
            ...
            // Field identifiers
            fix_child_element(key, 'conditions', element, row.index(), element.index());

            // Field values
            fix_child_values(key, true, 'condition', row, row.index(), element, element.index());

            // Fix meta field condition
            element.find('.rp_wcdpd_' + key + '_condition_method').change(function() {
                condition_type = element.find('.rp_wcdpd_' + key + '_condition_type').val();
                fix_meta_field_condition(key, element, condition_type);
            });

            // Fix coupons applied condition
            element.find('.rp_wcdpd_' + key + '_condition_method').change(function() {
                condition_type = element.find('.rp_wcdpd_' + key + '_condition_type').val();
                fix_coupons_applied_condition(key, element, condition_type);
            });

            // Datepicker
            element.find('.rp_wcdpd_date').datepicker(rp_wcdpd.datepicker_config);
        }
     ...
     */
    function fix_meta_field_condition(key, element, condition_type)
    {
        ...
        // Get current method
        var current_method = element.find('.rp_wcdpd_condition_setting_fields_' + condition_type + ' .rp_wcdpd_' + key + '_condition_method').val();

        // Reference text field
        var text_field = element.find('.rp_wcdpd_condition_setting_fields_' + condition_type + ' .rp_wcdpd_' + key + '_condition_text');

        // Proceed depending on current method
        if (jQuery.inArray(current_method, ['is_empty', 'is_not_empty', 'is_checked', 'is_not_checked']) !== -1) {
            element.find('.rp_wcdpd_condition_setting_fields_' + condition_type).find('select').parent().removeClass('rp_wcdpd_condition_setting_fields_single').addClass('rp_wcdpd_condition_setting_fields_double');
            text_field.parent().css('display', 'none');
            clear_field_value(text_field);
            text_field.prop('disabled', true);
        }
        else {
            element.find('.rp_wcdpd_condition_setting_fields_' + condition_type).find('select').parent().removeClass('rp_wcdpd_condition_setting_fields_double').addClass('rp_wcdpd_condition_setting_fields_single');
            element.find('.rp_wcdpd_condition_setting_fields_' + condition_type).find('.rp_wcdpd_' + key + '_condition_text').parent().css('display', 'block');
     ...
     */
    function fix_coupons_applied_condition(key, element, condition_type)
    {
        ...
        var coupons_field = element.find('.rp_wcdpd_condition_setting_fields_' + condition_type + ' .rp_wcdpd_' + key + '_condition_coupons');

        // Proceed depending on current method
        if (jQuery.inArray(current_method, ['at_least_one_any', 'none_at_all']) !== -1) {
            element.find('.rp_wcdpd_condition_setting_fields_' + condition_type).find('select').parent().removeClass('rp_wcdpd_condition_setting_fields_single').addClass('rp_wcdpd_condition_setting_fields_triple');
            coupons_field.parent().css('display', 'none');
            clear_field_value(coupons_field);
            coupons_field.prop('disabled', true);
        }
        else {
            element.find('.rp_wcdpd_condition_setting_fields_' + condition_type).find('select').parent().removeClass('rp_wcdpd_condition_setting_fields_triple').addClass('rp_wcdpd_condition_setting_fields_single');
    ...
    /**
     * BOGO - Receive Products
     */
    function fix_bogo_receive_products(element)
    {
        element.find('.rp_wcdpd_product_pricing_field_bogo_receive_products').change(function() {
            toggle_bogo_receive_products_items_field(element);
        });
        toggle_bogo_receive_products_items_field(element);
    }

    /**
     * Toggle visibility of receive products items fields
     */
    function toggle_bogo_receive_products_items_field(element)
    {
        element.find('.rp_wcdpd_product_pricing_field_bogo_receive_products').each(function() {

            // Get current items type
            var current_type = jQuery(this).val();

            // Change width of pricing fields
            element.find('.rp_wcdpd_product_pricing_field_bogo_pricing_method, .rp_wcdpd_product_pricing_field_bogo_pricing_value').each(function() {
                jQuery(this).closest('.rp_wcdpd_field').removeClass('rp_wcdpd_field_single rp_wcdpd_field_double').addClass((current_type === 'matched' ? 'rp_wcdpd_field_double' : 'rp_wcdpd_field_single'));
            });

            // Show items for current type
            element.find('.rp_wcdpd_field_bogo_items_' + current_type).each(function() {
                jQuery(this).css('display', 'block');
                toggle_form_field_visibility(jQuery(this), true);
            });

            // Hide all other items
            element.find('.rp_wcdpd_field_bogo_items').not('.rp_wcdpd_field_bogo_items_' + current_type).has('select:enabled').each(function() {
                jQuery(this).css('display', 'none');
                toggle_form_field_visibility(jQuery(this), false);
            });
        });
    ...
    function toggle_product_pricing_settings(element, key)
    {
        ...
        // Fix bogo inputs
        if (current_method === 'bogo' || current_method === 'bogo_repeat') {
            toggle_bogo_receive_products_items_field(row);
        }
